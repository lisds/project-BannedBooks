...
This plot highlights that the logistic regression equation's predictions are distributed more widely for defendants who are recidivists. We can also infer that the average logistic regression prediction is higher for recidivists than non-recidivists.
...
vad_df['V'] = vad_df['Words'].apply(lambda x: x[1])
vad_df['A'] = vad_df['Words'].apply(lambda x: x[2])
vad_df['D'] = vad_df['Words'].apply(lambda x: x[3])
...
charges = df['c_charge_desc'].value_counts().index.str.split()
```
